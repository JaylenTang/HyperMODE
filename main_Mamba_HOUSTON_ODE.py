import torch
from models import hypermode

import os
from config_HOUSTON import config
...
def loadData():
    if config.data == 'Houston2013':
        # Try different possible file names
        data_files = ['./data/Houston2013/GRSS2013.mat', './data/Houston2013/houston2013.mat']
        gt_files = ['./data/Houston2013/GRSS2013_gt.mat', './data/Houston2013/houston2013_gt.mat']
        ...
        if data_file is None or gt_file is None:
            raise FileNotFoundError(f"Data files not found. Tried: {data_files} and {gt_files}")
        
        # Try h5py first for MATLAB v7.3 files
        try:
            with h5py.File(data_file, 'r') as f:
                # Check for different possible key names
                if 'ori_data' in f.keys():
                    data = np.array(f['ori_data']).transpose((1, 2, 0))
                elif 'GRSS2013' in f.keys():
                    data = np.array(f['GRSS2013']).transpose((1, 2, 0))
                else:
                    # Get first non-meta key
                    keys = [k for k in f.keys() if not k.startswith('__')]
                    if not keys:
                        raise KeyError(f"Unknown key in data file. Available keys: {list(f.keys())}")
                    data = np.array(f[keys[0]]).transpose((1, 2, 0))

            with h5py.File(gt_file, 'r') as f:
                if 'map' in f.keys():
                    labels = np.array(f['map'])
                elif 'GRSS2013_gt' in f.keys():
                    labels = np.array(f['GRSS2013_gt'])
                else:
                    # Get first non-meta key
                    keys = [k for k in f.keys() if not k.startswith('__')]
                    if not keys:
                        raise KeyError(f"Unknown key in label file. Available keys: {list(f.keys())}")
                    labels = np.array(f[keys[0]])
            ...
            try:
                ...
                if 'GRSS2013' in data_mat:
                    data = data_mat['GRSS2013']
                elif 'houston2013' in data_mat:
                    data = data_mat['houston2013']
                else:
                    # Get first non-meta key
                    keys = [k for k in data_mat.keys() if not k.startswith('__')]
                    data = data_mat[keys[0]]
                
                if 'GRSS2013_gt' in gt_mat:
                    labels = gt_mat['GRSS2013_gt']
                elif 'houston2013_gt' in gt_mat:
                    labels = gt_mat['houston2013_gt']
                else:
                    # Get first non-meta key
                    keys = [k for k in gt_mat.keys() if not k.startswith('__')]
                    labels = gt_mat[keys[0]]
            except Exception as e2:
                print(f"Error loading with scipy.io: {e2}")
                raise

    elif config.data == 'WHU-Hi-LK':
        data = sio.loadmat('./data/WHU-Hi-LK/WHU_Hi_LongKou.mat')['WHU_Hi_LongKou']
        labels = sio.loadmat('./data/WHU-Hi-LK/WHU_Hi_LongKou_gt.mat')['WHU_Hi_LongKou_gt']
...
def train(train_loader):
    net = hypermode.VisionMambaODE(
        ...
        ode_integration_time=0.5,
        ode_method="euler",
        ode_hidden_dim=config.embed_dim // 4,
    ).cuda()
    # Use cross entropyloss function
    criterion = nn.CrossEntropyLoss()
    # Initializes the optimizer
    optimizer = optim.Adam(net.parameters(), lr=config.init_lr)
    # Start training
    total_loss = 0
    ...
    net.eval()  # 确保模型在 eval 模式下计算 FLOPs（与其他方法保持一致）
    from thop import profile
    # 与其他数据集脚本保持一致：使用训练过程中最后一个 batch 的第一个样本计算 FLOPs/Params
...
if __name__ == '__main__':
    ...
    log = Logger()
    log.open(config.logs + config.data + '_ODE_log.txt', mode='a')
    ...
    for num in range(config.test_epoch):
        ...
        save_path = config.checkpoint_path + 'ODE_TestEpoch%.f_OA%.3f_AA%.3f_Kappa%.3f/' % (num+1, each_oa, each_aa, each_kappa)
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        # torch.save(net.state_dict(), save_path + 'TestEpoch%.f_OA%.3f_AA%.3f_Kappa%.3f.pth.tar' % (num+1, each_oa, each_aa, each_kappa))
        get_cls_map_PU.get_cls_map(net, all_data_loader, y, save_path, 'Houston2013_SSPredictions.png')
    ...
    with open((config.logs + 'ODE_AVG_OA%.3f_AA%.3f_Kappa%.3f.txt' % (np.mean(oa), np.mean(aa), np.mean(kappa))), 'w') as file:
