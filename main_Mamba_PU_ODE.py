...
def train(train_loader):
    # HyperMODE: Mamba + Spectral ODE + Multi-scale Fusion
...
if __name__ == '__main__':
    ...
    for num in range(config.test_epoch):
        ...
        get_cls_map_PU.get_cls_map(net, all_data_loader, y, save_path, 'PU_SSPredictions.png')
    ...
    # BENCHMARK输出（最后一次运行）
    if len(oa) > 0:
        print('BENCHMARK_TRAIN_TIME=%.4f' % (toc1 - tic1))
        print('BENCHMARK_TEST_TIME=%.4f' % (toc2 - tic2)) 